...
zz0 <- Sys.time()		
		...
		dim_X <- base::dim(X)
		# XV1 = t(X) * V1
		XV1 <- base::matrix( 0 , nrow=dim_X[2] , ncol=dim_X[1] )
		for (gg in 1:G){
		    ind_gg <- id_list[[gg]]
			XV1[ , ind_gg] <- base::crossprod( X[ ind_gg , ] , V1_list[[gg]] )
		}
		# V1 * X = t( t(X) * t(V1) ) = t( t(X) * V1 ) = t( XV1 )
		XV2 <- base::solve( XVX ) %*% XV1
#			P <- V1 - V1 %*% X %*% XV2		
		P <- V1 - base::crossprod( XV1 , XV2 )
	    ...
	    for (pp in 1:NT){
			for (qq in pp:NT){
				...
				D1_V_qq <- mlnormal_fill_matrix_from_list( V1=V1zero , V1_list=D1_V_list[[qq]] , 
									  id_list=id_list , G =G )										
				theta_infomat[pp,qq] <- 0.5 * base::sum( ( P %*% D1_V_pp ) * ( P %*% D1_V_qq ) )
...
#	cat("-- infomat without loops ") ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1	
	    ...
	    for (pp in 1:NT){
			for (qq in pp:NT){
				# pp <- 1
				# qq <- 2		
				P0 <- base::matrix( 0 , nrow=N , ncol=N )
				for (gg in 1:G){
					for (hh in 1:G){
						# gg <- 1
						# hh <- 1
				...
				theta_infomat[pp,qq] <- 0.5 * base::sum(  P0 * P )
				theta_infomat[qq,pp] <- theta_infomat[pp,qq]
			}
		}
#	cat("-- infomat with loops ") ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1					

		res <- base::list( "theta_infomat" = theta_infomat )
		base::return(res)
}
######################################################################