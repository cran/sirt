...
xxirt <- function( dat , Theta = NULL , itemtype = NULL , customItems = NULL ,
		...
		res <- xxirt_data_proc(dat=dat , group = group )
		    ...
		    group0 <- res$group0 ; groups_unique <- res$groups_unique ; I <- res$I
		    maxK <- res$maxK ; ncat <- res$ncat ; weights <- res$weights
		    group_index <- res$group_index ; dat_resp <- res$dat_resp ;
		    resp_index <- res$resp_index ; dat1 <- res$dat1 
...
# cat(" - mstep theta ") ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1				
		...
		res <- xxirt_postproc_parameters( partable=partable , customTheta=customTheta , 
			...
			par_items <- res$par_items ; par_Theta <- res$par_Theta ;
			probs_items <- res$probs_items ; par_items_summary <- res$par_items_summary
