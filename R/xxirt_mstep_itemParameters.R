...
# print( paste0( "sum(pen) in likelihood = " , sum(pen) ))
			return(2*ll)
					}
...
# print( paste0( "sum(pen) in gradient = " , sum(pen) ))			
			pen_h <- xxirt_mstep_itemParameters_evalPrior(partable0, h=h)
# print( paste0( "sum(pen_h) in gradient = " , sum(pen_h) ))				
...
#		mod <- stats::optim( par = par0 , fn = like_items , method = mstep_method , 
#		                   control= arg_control )
