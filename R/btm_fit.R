...
btm_fit <- function( probs , dat0 , ind1 , ind2 , TP){			
			...
			Z_1 <- ( dat0[,3] - X_exp1 ) / base::sqrt( X_var1 )
	
			# second individual
			X_exp2 <- probs[,2]*1 + 1/2*probs[,3]
			X_var2 <- 1*probs[,2] + 1/4*probs[,3]
			X_var2 <- X_var2 - X_exp2^2
			Z_2 <- ( 1 - dat0[,3] - X_exp2 ) / base::sqrt( X_var2 )
			
			# compute outfit statistic
			out1 <- base::rowsum( Z_1^2 , dat0[,1] )
			N1 <- base::rowsum( 1+0*Z_1 , dat0[,1] )
			out2 <- base::rowsum( Z_1^2 , dat0[,2] )
			N2 <- base::rowsum( 1+0*Z_1 , dat0[,2] )
			wvar1 <- base::rowsum( X_var1 , dat0[,1] )
			wvar2 <- base::rowsum( X_var2 , dat0[,2] )
			win1 <- base::rowsum( X_var1*Z_1^2 , dat0[,1] )
			win2 <- base::rowsum( X_var2*Z_1^2 , dat0[,2] )
			...
			res0 <- base::list( outfit = out / N ,  infit = win / wvar )
			base::return(res0)
				}
#########################################################################				
