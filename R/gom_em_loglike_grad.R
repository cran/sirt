        ...
        weights, lambda_partable, lambda_index_blocks)
{
    ...
    for (jj in 1:B){
        ...
        lambda <- gom_em_loglike_parameter_conversion( x=x1, ind_lambda=ind_lambda,
                    ind_pi=ind_pi, I=I, K=K, ind_mu=ind_mu, ind_sigma=ind_sigma, model=model,
                    theta_grid=theta_grid, lambda_partable=lambda_partable )$lambda
        ...
        ll2 <- sirt_rcpp_gom_em_loglike_gradient( probs=probs0, probs_h=probs_h, ncat=ncat,
                    TP=TP, dat2=dat2, dat2resp=dat2.resp, pik=pi.k, items=items, fyiqk=fyiqk,
                    weights=weights)
        grad1[ind] <- (ll2 - ll) / h
    }
